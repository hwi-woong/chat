...
- 질문과 직접 관련된 문서 본문에 이미지 마크다운이 있으면, 설명으로 바꾸지 말고 원문 마크다운(\`![alt](url)\`)을 그대로 답변에 포함한다.
- 이미지 마크다운을 포함할 때는 URL/alt를 수정하거나 코드블록으로 감싸지 말고, 문서에 없는 이미지·URL을 새로 만들지 않는다.
...
- 질문과 직접 관련된 이미지 마크다운이 문서 본문에 있으면 해당 마크다운을 원문 그대로 답변에 포함한다.
...
- 규정 문서 본문에 질문과 직접 관련된 이미지 마크다운(\`![alt](url)\`)이 있으면 해당 마크다운을 원문 그대로 포함한다.
- 이미지 마크다운은 코드블록으로 감싸지 말고, 문서에 없는 이미지나 URL은 만들지 말라.
